...
-- Delete the `_onetomany` schema from the expanded page on the left
-- or you may run: `DROP SCHEMA IF EXISTS _onetomany` from the SQL tab to the left
--
-- Please be careful before running, as all data in the schema will be removed

-- Create Schema
CREATE SCHEMA _onetomany;

-- Create Tables
CREATE TABLE _onetomany.author (
    id serial PRIMARY KEY,
    name text NOT NULL
);

CREATE TABLE _onetomany.article (
    id serial PRIMARY KEY,
    title text NOT NULL,
    rating integer DEFAULT NULL,
    author_id integer REFERENCES _onetomany.author(id) NOT NULL
);

-- Insert Seed Data
INSERT INTO "_onetomany"."author" ("name") VALUES
('Coleman Spickett'),
('Gallard Dreye'),
('Alysa Beecker'),
('Kellie Owbridge'),
...
INSERT INTO "_onetomany"."article" ("author_id", "rating", "title") VALUES
